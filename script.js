// ===== CONFIGURACIÓN MEDELLÍN SUR =====
const CONFIG_MEDELLIN_SUR = {
    RUTA: "Medellín Sur",
    TABLAS_SUPABASE: {
        conductores: 'conductores_medellin_sur',
        vehiculos: 'vehiculos_medellin_sur', 
        tablas: 'tablas_medellin_sur',
        viajes: 'viajes_medellin_sur'
    },
    TIEMPOS_VIAJE: {
        "La Ceja - Medellín Sur": 110,
        "Medellín Sur - La Ceja": 110
    },
    CONFIG_PUNTOS_CONTROL: {
        "La Ceja - Medellín Sur": [
            {"id": "tramo_memos", "label": "CIT-Los Memos", "duracion": 5},
            {"id": "tramo_terminal_sur", "label": "Terminal del Sur", "duracion": 105}
        ],
        "Medellín Sur - La Ceja": [
            {"id": "tramo_exposiciones", "label": "Terminal del Sur-Punto Clave", "duracion": 0, "tipo": "punto_clave"},
            {"id": "tramo_ceja", "label": "Exposiciones-La Ceja", "duracion": 110}/// reviso esta duracion
        ]
    }
};
...
async function cargarSelectores() {
    ...
    try {
        // Cargar Conductores - MEDELLÍN SUR
        const { data: conductores, error: errorConductores } = await supabaseClient
            .from(CONFIG_MEDELLIN_SUR.TABLAS_SUPABASE.conductores)
            .select('id, nombre')
            .order('nombre');
        ...
        // Cargar Vehículos - MEDELLÍN SUR
        const { data: vehiculos, error: errorVehiculos } = await supabaseClient
            .from(CONFIG_MEDELLIN_SUR.TABLAS_SUPABASE.vehiculos)
        ...
        // Cargar Tablas - MEDELLÍN SUR
        const { data: tablas, error: errorTablas } = await supabaseClient
            .from(CONFIG_MEDELLIN_SUR.TABLAS_SUPABASE.tablas)
        ...
        console.log('✅ Selectores de Medellín Sur cargados correctamente');
    } catch (error) {
        console.error('❌ Error al cargar selectores Medellín Sur:', error);
...
function actualizarPuntosControl(ruta) {
    ...
    if (!ruta || !CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta]) return;

    const puntos = CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta];
    let tiempoAcumulado = 0;

    puntos.forEach((punto, index) => {
        tiempoAcumulado += punto.duracion;
        const esUltimo = index === puntos.length - 1;
        const esPuntoClave = punto.tipo === 'punto_clave';
        ...
        // ✅ CORREGIDO: Para el último punto, el campo de hora real SIEMPRE visible
        if (esPuntoClave) {
            puntoDiv.innerHTML = `
                <div class="form-group">
                    <label>
                        <span id="proyeccion-label-${punto.id}">${labelFinal}</span>
                        <div class="radio-group">
                            <label><input type="radio" name="estado_${punto.id}" value="a_tiempo" checked> Sí pasó</label>
                            <label><input type="radio" name="estado_${punto.id}" value="tarde"> No pasó</label>
                        </div>
                    </label>
                </div>
                <div id="detalle-${punto.id}" style="display: none;">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="motivo_${punto.id}">Motivo por el cual no pasó:</label>
            ...
            // ✅ CORREGIDO: Para el último punto, campo de hora SIEMPRE visible
            puntoDiv.innerHTML = `
                <div class="form-group">
                    <label>
                ...
                <div id="detalle-${punto.id}">
                    <!-- ✅ CAMBIO: Para último punto, SIEMPRE mostrar campo de hora real -->
                    <div class="form-group" style="display: ${esUltimo ? 'block' : 'none'};">
                    ...
                    <!-- ✅ Los campos de justificación SOLO se muestran cuando es "tarde" -->
                    <div id="justificacion-${punto.id}" style="display: none;">
                        <div class="form-group">
                            <label>¿Fue justificado?</label>
                            <div class="radio-group">
                                <label><input type="radio" name="justificado_${punto.id}" value="si"> Sí</label>
        ...
        // Evento para cambio de estado - CORREGIDO
        ...
        radios.forEach(radio => {
            radio.addEventListener('change', function() {
                if (esPuntoClave) {
                    ...
                    if (detalle) {
                        detalle.style.display = this.value === 'tarde' ? 'block' : 'none';
                    }
                } else {
                    // Para puntos normales
                    const justificacionDiv = document.getElementById(`justificacion-${punto.id}`);
                    const horaRealDiv = puntoDiv.querySelector(`[id^="hora_real_${punto.id}"]`)?.parentElement;
                    
                    if (justificacionDiv) {
                        justificacionDiv.style.display = this.value === 'tarde' ? 'block' : 'none';
                    }
                    
                    // ✅ Para el último punto, la hora real SIEMPRE visible
                    if (horaRealDiv && !esUltimo) {
                        horaRealDiv.style.display = this.value === 'tarde' ? 'block' : 'none';
                    }
                    
                    // Limpiar campos si se cambia a "a tiempo"
                    if (this.value === 'a_tiempo' && !esUltimo) {
                        ...
                        const motivoSelect = document.getElementById(`motivo_${punto.id}`);
        ...
        // Evento para hora real - SOLO para puntos normales
        if (!esPuntoClave) {
                ...
                horaRealInput.addEventListener('change', function() {
                    ...
                    if (proyeccionSpan && proyeccionSpan.dataset.minutos) {
                        ...
                        if (!esUltimo) {
                            ...
                            const maxTiempo = CONFIG_MEDELLIN_SUR.TIEMPOS_VIAJE[ruta];
                            ...
                            if (tiempoTotal > maxTiempo) {
                                ...
                                if (justificacionDiv) justificacionDiv.style.display = 'block';
                            }
                        }
                    }
                });
...
function calcularProyecciones(ruta, salidaReal) {
    console.log('DEBUG RUTA SUR: Calculando proyecciones para ruta:', ruta, 'salida:', salidaReal);
    if (!ruta || !salidaReal || !CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta]) {
        console.error('DEBUG RUTA SUR: Ruta, salida o config inválida:', ruta, salidaReal, CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta]);
        return;
    }
    const puntos = CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta];
    let tiempoBase = timeToMinutes(salidaReal);
    console.log('DEBUG RUTA SUR: Tiempo base min:', tiempoBase);

    puntos.forEach((punto, index) => {
        // Saltar puntos clave (duración 0)
        if (punto.duracion > 0) {
        ...
        const proyeccionSpan = document.getElementById(`proyeccion-${punto.id}`);
        if (proyeccionSpan) {
            // Ocultar proyección para puntos clave
            if (punto.tipo === 'punto_clave') {
                proyeccionSpan.textContent = 'N/A';
                proyeccionSpan.style.display = 'none';
                document.getElementById(`proyeccion-label-${punto.id}`).textContent = punto.label;
            } else {
            ...
            const horaRealInput = document.getElementById(`hora_real_${punto.id}`);
            if (horaRealInput && horaRealInput.value && punto.tipo !== 'punto_clave') {
...
function validarLlegadaFinal(ruta) {
    ...
    if (!horaRealUltimo || !horaRealUltimo.value) {
        alert(`La hora real de llegada final (${CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta][CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[ruta].length - 1].label}) es obligatoria.`);
        return false;
    }
    return true;
}
        ...
        btnCalcular.addEventListener('click', async function() {
            ...
            try {
                const { data, error } = await supabaseClient
                    .from(CONFIG_MEDELLIN_SUR.TABLAS_SUPABASE.viajes)
                ...
                resultadoContainer.innerHTML = `
                    ...
                    <p style="margin-top: 1rem; color: var(--success-color);">
                        <i class="fas fa-check-circle"></i> Datos guardados exitosamente en ${CONFIG_MEDELLIN_SUR.RUTA}.
                    </p>
                `;
...
async function cargarInformeVehiculos() {
    ...
    try {
        const { data: viajes, error } = await supabaseClient
            .from(CONFIG_MEDELLIN_SUR.TABLAS_SUPABASE.viajes)
            .select(`vehiculo_id, vehiculos_medellin_sur (numero_interno), tipo_monitoreo, fecha_monitoreo`);
        if (error) throw error;
        ...
        viajes.forEach(viaje => {
            const vehiculo = viaje.vehiculos_medellin_sur?.numero_interno || 'Desconocido';  
...
async function cargarInformeAdvertencias() {
    ...
    try {
        const { data: viajes, error } = await supabaseClient
            ...
            .select(`
                *,
                conductores_medellin_sur (nombre),
                vehiculos_medellin_sur (numero_interno),
                tablas_medellin_sur (nombre_tabla)
            `);
    ...
    try {
        ...
        viajes.forEach(viaje => {
            ...
            if (hayInfraccionInjustificada) {
                ...
                // Registrar cuando llega a múltiplos de 5
...
function generarTablaDetalles(conductor, evidencias) {
    ...
    evidencias.forEach(viaje => {
        ...
        const vehiculo = viaje.vehiculos_medellin_sur?.numero_interno || 'N/A';
        const tabla = viaje.tablas_medellin_sur?.nombre_tabla || 'N/A';
...
async function cargarRegistros() {
    ...
    try {
        ...
        const { data: viajes, error } = await supabaseClient
            ...
            .select(`
                *,
                conductores_medellin_sur!inner (nombre),
                vehiculos_medellin_sur!inner (numero_interno),
                tablas_medellin_sur!inner (nombre_tabla)
            `)
            .order('created_at', { ascending: false });

        console.log('DEBUG: Respuesta Supabase - Data:', viajes?.length || 0, 'Error:', error);
        ...
        viajes.forEach(viaje => {
            ...
            const conductor = viaje.conductores_medellin_sur?.nombre || 'N/A';
            const vehiculo = viaje.vehiculos_medellin_sur?.numero_interno || 'N/A';
            const tabla = viaje.tablas_medellin_sur?.nombre_tabla || 'N/A';
            ...
            if ((retrasoSalida > 0 && !justificadoSalida) || hayPuntoInjustif) {
                ...
                const conductor = viaje.conductores_medellin_sur?.nombre || 'Desconocido';
                const vehiculo = viaje.vehiculos_medellin_sur?.numero_interno || '000';
                const tabla = viaje.tablas_medellin_sur?.nombre_tabla || '00';
                const fechaActual = new Date().toLocaleDateString('es-CO');

                const puntosControlConfig = CONFIG_MEDELLIN_SUR.CONFIG_PUNTOS_CONTROL[viaje.ruta] || [];
                let mensajePuntos = '';
                ...
                viaje.puntos_control_detalle.forEach(punto => {
                    ...
                    // Rutas de Medellín Sur
                    if (viaje.ruta === 'La Ceja - Medellín Sur') {
                        if (punto.label.includes('CIT-Los Memos')) labelAjustado = 'CIT-Los Memos';
                        else if (punto.label.includes('Terminal del Sur')) labelAjustado = 'Terminal del Sur';
                    } else if (viaje.ruta === 'Medellín Sur - La Ceja') {
                        if (punto.label.includes('Terminal del Sur-Exposiciones')) labelAjustado = 'Terminal del Sur-Exposiciones';
                        else if (punto.label.includes('Exposiciones-La Ceja')) labelAjustado = 'Exposiciones-La Ceja';
                    }

                    labelsPrevios.push(labelAjustado);
                    
                    // Lógica diferente para puntos clave
                    if (configPunto.tipo === 'punto_clave') {
                        // PUNTO CLAVE: Solo verificar si pasó o no
                        if (punto.estado === 'tarde' && punto.justificado === 'no') {
                            mensajePuntos += `No pasó por el punto de control ${labelAjustado}.\n`;
                        }
                    } else {
                        // PUNTO NORMAL: Lógica original con tiempos
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM cargado, inicializando aplicación Medellín Sur...');
    ...
    console.log('Aplicación Medellín Sur inicializada correctamente');
});